...
import "./CSS/Hardware.css"; // reuse same styles
...
useEffect(() => {
    ...
    // Compose data to send to parent
    if (checked) {
      onSectionDataChange({
        title,
        complexity,
...
const Software = ({ onCostChange , onDataChange}) => {
...
useEffect(() => {
  ...
  // Replace these with your software-specific addons if needed
  const addons1 = [];
  const addons2 = [];
  const addons3 = [];
  const addons4 = [];
...
/>
<Section
  title="Custom Linux OS (BSP) Development"
  addons={addons4}
  onSectionCostChange={(cost) => handleSectionCostChange(3, cost)}
  onSectionDataChange={(data) => handleSectionDataChange(3, data)}
/>
    </div>
  )
};
