...
const Mechanical = ({ onCostChange , onDataChange }) => {
 ...
 useEffect(() => {
  ...
  const industrialDesignAddons = []
  const mechanicalEnclosureAddons = []

  return (
    <div className="hardware-wrapper">
      <Section
        title="Industrial Design (Concepts)"
        addons={industrialDesignAddons}
        onSectionCostChange={(cost) => handleSectionCostChange(0, cost)}
        onSectionDataChange={(data) => handleSectionDataChange(0, data)}
      />
      <Section
        title="Mechanical Enclosure Design (CAD)"
        addons={mechanicalEnclosureAddons}
